...
PlayerShader::PlayerShader() {
    ShaderSource shader_source =
        hookline::load_shader_file("BasicMeshShader.vert", "PlayerShader.frag");
...
PlayerShader::PlayerShader(PlayerShader&& other) noexcept : m(other.m) {
    other.m.program = 0;
}

PlayerShader& PlayerShader::operator=(PlayerShader&& other) noexcept {
    if (this == &other) {
        return *this;
    }
...
void PlayerShader::updateUniforms(glm::vec2 user_pos, glm::vec2 u_scale,
                                  float u_rotation, glm::vec2 camera_pos,
                                  glm::vec2 camera_viewport_size,
                                  float camera_pixels_per_unit, float time,
                                  float health) {
    // Uniforms
    // -- Vertex shader
