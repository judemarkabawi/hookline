...
// only need first 2 params, default is 2D tex, w vertical wrap and linear
// mipmap filtering
GLuint AssetManager::load_texture(const std::string &name,
                                  const std::string &filename,
                                  const GLenum texType, const GLint filterType,
    ...
    if (texType == GL_TEXTURE_CUBE_MAP) {
        load_png(filename + "posx.png", &size, &data,
                 OriginLocation::LowerLeftOrigin);
        glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X, 0, GL_RGBA, size.x, size.y,
                     0, GL_RGBA, GL_UNSIGNED_BYTE, data.data());
        load_png(filename + "posy.png", &size, &data,
                 OriginLocation::LowerLeftOrigin);
        glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_Y, 0, GL_RGBA, size.x, size.y,
                     0, GL_RGBA, GL_UNSIGNED_BYTE, data.data());
        load_png(filename + "posz.png", &size, &data,
                 OriginLocation::LowerLeftOrigin);
        glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_Z, 0, GL_RGBA, size.x, size.y,
                     0, GL_RGBA, GL_UNSIGNED_BYTE, data.data());
        load_png(filename + "negx.png", &size, &data,
                 OriginLocation::LowerLeftOrigin);
        glTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_X, 0, GL_RGBA, size.x, size.y,
                     0, GL_RGBA, GL_UNSIGNED_BYTE, data.data());
        load_png(filename + "negy.png", &size, &data,
                 OriginLocation::LowerLeftOrigin);
        glTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_Y, 0, GL_RGBA, size.x, size.y,
                     0, GL_RGBA, GL_UNSIGNED_BYTE, data.data());
        load_png(filename + "negz.png", &size, &data,
                 OriginLocation::LowerLeftOrigin);
        glTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_Z, 0, GL_RGBA, size.x, size.y,
                     0, GL_RGBA, GL_UNSIGNED_BYTE, data.data());
    } else {
