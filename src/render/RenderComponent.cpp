...
/**
 * Make a new RenderComponent using vertices and a (default) color. Default type
 * is BASE
 */
RenderComponent RenderComponent::from_vertices_color(
    const std::vector<glm::vec2>& vertices, glm::vec4 color, RenderType type) {
...
/**
 * Make a new RenderComponent using vertices, (default) color, and tex_cords
 * (but no texture). Default type is BASE
 */
RenderComponent RenderComponent::from_vertices_color_tex(
    const std::vector<glm::vec2>& vertices, glm::vec4 color,
    const std::vector<glm::vec2>& tex_coords, RenderType type) {
...
RenderComponent RenderComponent::from_vertices_texture(
    ...
    const std::vector<glm::vec2>& tex_coords, GLuint texture, RenderType type) {
    assert(vertices.size() == tex_coords.size() &&
           "RenderComponent: Cannot construct with different length vertices "
           "and tex_coords");
